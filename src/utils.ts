...
export function pluckData<R>(obj: MaybeData<R>): R {
    ...
    throw new Error(
        `Expected response object with a "data" property, but received: ${obj}`
    );
}
...
export function catchNotFoundOrThrow(err: ApifyApiError): void {
    ...
    const isNotFoundMessage =
        err.type === RECORD_NOT_FOUND_TYPE ||
...
export function parseDateFields(
    ...
    if (Array.isArray(input))
        return input.map((child) =>
            parseDateFields(child, shouldParseField, depth + 1)
        );
 ...
 */
export function stringifyWebhooksToBase64(
...
export async function maybeGzipValue(
    value: unknown
    ...
    if (encoded.byteLength < MIN_GZIP_BYTES) return encoded;
...
function toStream(input: Uint8Array): ReadableStream<Uint8Array> {
 ...
 */
export function sliceArrayByByteLength<T>(
    array: T[],
    maxByteLength: number,
    startIndex: number
): T[] {
    const stringByteLength = (str: string) =>
        isNode() ? Buffer.byteLength(str) : new Blob([str]).size;
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if (itemByteSize > maxByteLength) {
            throw new Error(
                `RequestQueueClient.batchAddRequests: The size of the request with index: ${
                    startIndex + i
                } ` +
...
export function isBuffer(
    value: unknown
): value is Buffer | ArrayBuffer | TypedArray {
    ...
    async *[Symbol.asyncIterator](): AsyncIterator<RequestQueueClientListRequestsResult> {
        ...
        while (true) {
            const pageLimit = this.limit
                ...
                : this.maxPageLimit;
            const pageExclusiveStartId =
                nextPageExclusiveStartId || this.exclusiveStartId;
            const page: RequestQueueClientListRequestsResult =
                await this.getPage({
                    limit: pageLimit,
...
export type DistributiveOptional<T, K extends keyof T> = T extends any
